...
After Orukeet returns text, a second model turns the spoken wording into a written sentence: fillers and false starts come out, punctuation goes in. The model is [[https://huggingface.co/mlx-community/Qwen3.5-0.8B-4bit-OptiQ][Qwen3.5-0.8B]] at mixed 4-bit, about 0.6 GB on disk and about 1.5 GB in memory for a short clip. That is the newest Qwen that still fits beside Orukeet. Qwen 3.6 and Qwen 3.8 start at tens of billions of parameters. NVIDIA's smallest current Nemotron Nano is 4B, about 2.3 GB at 4-bit, which crowds the speech model on an 8 GB Air.
...
Orukeet writes down what it heard, including the parts you would not type. "Um, I want to, I want the report" should become "I want the report." The small language model does that edit. It must leave the speaker's words alone: no summary, no synonym, no new fact.

Qwen 2.5 0.5B was an earlier choice because a 4-bit copy was small and =mlx-lm= already knew how to load it. Qwen3.5-0.8B is the later small model in that family. Qwen 3.6 has no model under 27B. Qwen 3.8-Flash is a 125B mixture-of-experts. Both are the wrong size for this laptop. NVIDIA Nemotron models are strong, and the smallest Nano is still 4B, about 2.3 GB at 4-bit. Hugging Face does have a Qwen3-0.6B fine-tune whose only job is spoken-to-written cleanup, =Aye10032/Qwen3-0.6B-ASR-Refiner=, trained on Chinese. This dictation is multilingual, and that checkpoint is Chinese, so the general Qwen3.5-0.8B is the one we load. The prompt tells it the job.

The guard is what we trust when the model wanders. =apply_cleanup= keeps the model's line only when every word it produced already belongs to the transcript (after expanding a few spoken forms such as "gonna" into "going to"), the words stay in order, and it has not thrown away most of the sentence. Anything else is pasted as Orukeet wrote it. Qwen3.5 thinks out loud unless we pass =enable_thinking=False=. A leftover think block is stripped before that check.

=Cleaner= loads the MLX weights once. =mlx_lm= is imported inside =load=, so this file imports on a machine that does not have MLX. The first launch downloads about 0.6 GB into the Hugging Face cache. =mlx-lm= 0.30.7 or newer knows the Qwen3.5 architecture.
...
The tests pass a fake model output into =apply_cleanup=. They never load Qwen.
...
=setup= loads Qwen once, before the hotkey is armed, so the first sentence does not wait on a download. If that load fails, dictation still pastes Orukeet's text. =finish= cleans on the background thread, next to the HTTP call, then posts the paste back to the main thread.
...
=install-orukeet.sh= creates a virtualenv, installs the Orukeet wheel, installs PyObjC and =mlx-lm= on macOS, and writes =installation.json= for the Q8 model. =run.sh= starts the agent, which starts the worker and loads Qwen. The Qwen weights download on first launch. Set =ORUKEET_CLEANUP=0= to paste Orukeet's text with no rewrite.
...
# mlx-lm 0.30.7 knows the Qwen3.5 architecture used by the cleanup model.
pyobjc-core>=10.3
