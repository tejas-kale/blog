...
On an 8 GB M1 MacBook Air, use the native Q8 weights. That file is =orukeet-v0.1.0-q8.gguf=, about 714 MB. =orukeet install= verifies it and picks the Metal runtime. An ONNX INT8 export is a bit smaller, and it is a different runtime, so the worker stays on the native file. Run one worker. Recordings stop at 60 seconds. Orukeet's fast Metal path covers clips up to about 41 seconds; longer clips still transcribe.

Orukeet already writes casing and punctuation. The cleanup step is narrower than "make this readable." It removes fillers, resolves a self-correction ("Thursday, no, Friday"), and applies a spoken command ("new paragraph"). The model for that, on this laptop, is [[https://huggingface.co/SpeakoFlow/speakoflow-mini][SpeakoFlow Mini 0.8B]], the Q4_K_M file, 542 MB. It is a Qwen3.5-0.8B fine-tune whose only job is English dictation cleanup. The author's reference build is Q8_0, 833 MB. On this machine that file sits beside Orukeet's own Q8 weights, so we load Q4_K_M. Their evaluation puts Q4 1.3 points behind Q8, and 7.3% of outputs differ. Set =ORUKEET_CLEANUP_FILE= if you want the Q8 file instead.

Two programs cooperate:

1. =server/orukeet_server.py= loads Orukeet and answers HTTP on =127.0.0.1:8765=.
2. =python -m dictation= watches the key, records, posts the WAV, cleans the transcript, and pastes the text.
...
* Spoken words, written sentence

Cleanup is two stages. A rule is the right tool when the decision is mechanical. A model is the right tool when the decision needs a judgement, and only then.

Stage one, =apply_rules=, drops filler words, collapses an immediate repetition ("the the"), and turns "new paragraph" and "new line" into breaks. It leaves "like" alone, and it does not turn "three" into "3". A bare retraction such as "Thursday no Friday" is also left alone. Those are judgement calls.

Stage two is [[https://huggingface.co/SpeakoFlow/speakoflow-mini][SpeakoFlow Mini]]. The system prompt below is the string it was fine-tuned on. The user message is the transcript and nothing else. Temperature is 0. We do not put a tight cap on =max_tokens=: a cut-off long transcript is lost content. The ceiling of 4096 tokens is only there so a runaway reply can be detected and thrown away. If the reply stops because it hit that ceiling, we paste the rules text.

The failure mode to design for is the model answering the transcript instead of cleaning it. =apply_cleanup= rejects a reply that opens like a chatbot ("Sure!", "Here is", "You should", "Of course") and a reply longer than both three times the input and the input plus 200 characters. Anything rejected, or an empty reply, falls back to the rules text. A leftover think block is stripped first. We do not also demand that every output word already appeared in the input. That lock would discard the edits this model is for: "Thursday, no, Friday" becoming "Friday", or "sam at example dot com" becoming an email address.

A long clip is cleaned per Orukeet segment, then joined. One segment is cleaned as a whole. The weights stay loaded. =llama_cpp= and =huggingface_hub= are imported inside =load=, so this file imports on a machine that does not have them. The GGUF is what the published evaluation measured. There is no MLX build of this checkpoint.

=ORUKEET_CLEANUP=rules= runs stage one and skips the model. =ORUKEET_CLEANUP=0= pastes Orukeet's text.

#+begin_src python :tangle dictation/cleanup.py
...
The tests call the rules and the guard with fake model output. They never download the GGUF.

#+begin_src python :tangle tests/test_cleanup.py
...
This is the module you run. It owns one recorder, one hotkey, the Orukeet worker, and the cleanup model.
...
=setup= loads SpeakoFlow once, before the hotkey is armed, so the first sentence does not wait on a download. If that load fails, the rules still run. =finish= cleans on the background thread, next to the HTTP call, then posts the paste back to the main thread.
...
=install-orukeet.sh= creates a virtualenv, installs the Orukeet wheel, installs PyObjC and =llama-cpp-python= on macOS, and writes =installation.json= for the Q8 speech model. =run.sh= starts the agent, which starts the worker and loads SpeakoFlow. That GGUF downloads on first launch. Set =ORUKEET_CLEANUP=rules= to skip the language model. Set =ORUKEET_CLEANUP=0= to paste Orukeet's text with no rewrite.
...
# macOS only. PyObjC is the hotkey, the microphone, and Command-V.
# llama-cpp-python loads the SpeakoFlow GGUF. huggingface_hub downloads it.
pyobjc-core>=10.3
