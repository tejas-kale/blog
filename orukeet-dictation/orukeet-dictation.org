...
After Orukeet returns text, a second model turns the spoken wording into a written sentence: fillers and false starts come out, punctuation goes in. The model is [[https://huggingface.co/mlx-community/Qwen2.5-0.5B-Instruct-4bit][Qwen2.5-0.5B-Instruct]] in a 4-bit MLX build, about 278 MB on disk and under 1 GB once loaded for a short clip. A 3B model at the same precision already peaks near 2 GB on an M1 Air, which is too much beside Orukeet. The 0.5B model is the same kind of size as Orukeet, and it only has to edit a sentence it has already been given.
...
A 0.5B model will sometimes ignore that instruction. The guard in this module is what we trust. =apply_cleanup= keeps the model's line only when every word it produced already belongs to the transcript (after expanding a few spoken forms such as "gonna" into "going to"), the words stay in order, and it has not thrown away most of the sentence. Anything else is pasted as Orukeet wrote it.

=Cleaner= loads the MLX weights once. =mlx_lm= is imported inside =load=, so this file imports on a machine that does not have MLX. The first launch downloads about 278 MB into the Hugging Face cache.
...
# mlx-lm runs the 0.5B cleanup model on Apple silicon.
pyobjc-core>=10.3
